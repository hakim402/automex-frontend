...
export function PartnersClientPage({
  ...
  function buildQuery(overrides: {
    ...
    const pt =
      overrides.type !== undefined ? overrides.type : activePartnerType;
    ...
    <div className="relative overflow-hidden ">
      ...
      <div className="mx-auto max-w-7xl px-4 py-16 sm:py-24 mt-10 md:mt-20">
        ...
        <p className="text-center text-[12px] text-muted-foreground mb-10">
          {partners.length === 1
            ? t("listing.count.singular", { count: partners.length })
            : t("listing.count.plural", { count: partners.length })}
        </p>

        {partners.length === 0 ? (
          <div className="text-center py-20">
            <div className="mb-4 opacity-30">
              <Handshake className="size-10 mx-auto" aria-hidden="true" />
            </div>
                  ...
                  {items.map((partner) => {
                    ...
                    return (
                      <Link
                        ...
                        className="group relative flex flex-col items-center gap-3 rounded-2xl border border-border/40 bg-card/50 backdrop-blur-sm p-6 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-brand/5 hover:border-primary/40 cursor-pointer"
                      >
                        {/* Logo */}
                        {partner.logo?.url ? (
                        ...
                        {/* External website link – fixed nested <a> issue */}
                        {partner.website_url && (
                          <span
                            role="button"
                            tabIndex={0}
                            onClick={(e) =>
                              handleExternalLink(e, partner.website_url!)
                            }
                            onKeyDown={(e) => {
                            ...
                            className="absolute top-3 right-3 inline-flex items-center gap-1 text-[10px] text-muted-foreground/50 hover:text-primary transition-colors bg-background/50 rounded-full px-2 py-0.5 backdrop-blur-sm cursor-pointer"
