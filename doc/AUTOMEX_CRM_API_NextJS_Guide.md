...
## 8. Write endpoints (CRM — contact, quotes, booking, newsletter)

Mounted at `/api/v1/crm/`, same `X-API-Key` header as everything else, but
throttled tighter (**20 requests/minute** per key vs. content's 300/min —
these are form submissions, not page loads).

| Method | Path | Body |
|---|---|---|
| POST | `/api/v1/crm/leads/contact/` | `{ full_name, email, phone?, company?, job_title?, message?, service_interest?, industry?, budget_range?, timeline?, utm_source?, ... }` |
| POST | `/api/v1/crm/leads/quote/` | Same as contact + `{ requested_services: [uuid], project_description?, estimated_budget_min?, estimated_budget_max?, currency? }` |
| GET | `/api/v1/crm/bookings/availability/?date=2026-08-03` | Returns open slots for that date |
| POST | `/api/v1/crm/bookings/consultations/` | Same as contact + `{ slot: uuid, scheduled_date, scheduled_time, meeting_type?, timezone?, notes? }` |
| POST | `/api/v1/crm/newsletter/subscribe/` | `{ email, language?, source? }` |

**`service_interest`, `industry`, `requested_services`, `slot` are UUIDs**
(the `id` field from the content/booking APIs), not slugs — resolve the
slug → id via the read API first (you're already fetching the service page
by slug, so you already have its `id`).

### Example — contact form

```ts
export async function submitContactForm(data: ContactFormInput) {
  return automexFetch<{ id: string; lead_type: string; status: string; created_at: string }>(
    "/leads/contact/",
...
```ts
// 1. Get open slots for the date the user picked
const slots = await automexFetch<AvailableSlot[]>(`/bookings/availability/?date=${date}`);

// 2. User picks a slot + time within its window, then:
await automexFetch("/bookings/consultations/", {
  method: "POST",
  body: JSON.stringify({
    full_name, email, slot: chosenSlot.id,
    scheduled_date: date, scheduled_time: chosenTime,
    meeting_type: "video",
  }),
...
A `400` on booking means the slot filled up between step 1 and step 2 (race
condition, handled server-side) — re-fetch availability and show the user
updated options rather than treating it as a generic error.

### Response shape (all write endpoints)

```json
{ "id": "uuid", "lead_type": "contact", "lead_type_display": "Contact Form", "status": "new", "created_at": "2026-07-12T10:00:00Z" }
```

Deliberately minimal — no `score`, `assigned_to`, or internal pipeline
fields are ever returned to a public caller.

### Validation errors

```json
{ "full_name": ["This field is required."], "email": ["Enter a valid email address."] }
```
Field-keyed, same shape DRF always uses — map directly to form field errors.
...
| `429` | Rate limit exceeded (300/min read, 20/min write) |
| `400` | Invalid filter/query param value, or write-endpoint validation failure |
